...
| `image_alt.py` | Not run standalone — imported by `holmes_post.py`. Generates the alt text for each post's image via a separate model call shown only the image's pixels, never the caption, so it describes what is visible rather than paraphrasing provenance. The description is checked against the image before it ships (a further call locates each claim; an unverifiable one gets a single retry, then the description is dropped). Any failure along the way falls back to a plain citation-only alt rather than holding the post. Both calls run `claude -p --restricted --tools Read` (`CONFINED`) since 11 September 2026: unconfined, `claude -p` is an agent with a shell, and in a sibling bot it was found cropping images through a dozen tool calls and once running the project's own code; confined, it can read the one image in its working directory and nothing else. |
