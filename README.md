...
| `image_alt.py` | Not run standalone — imported by `holmes_post.py`. Generates alt text via a model call shown only the image's pixels (never the caption), then verifies each claim against the image before shipping — an unverifiable claim gets one retry, then the description is dropped. Any failure falls back to a plain citation-only alt rather than holding the post. Runs sandboxed (`claude -p --restricted --tools Read`, `CONFINED`) since 11 September 2026, after an unconfined sibling bot was found using its shell access to crop images and run project code. |
